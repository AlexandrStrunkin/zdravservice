...
if (!empty($arResult["ELEMENTS"]) && CModule::IncludeModule("iblock"))
{
	...
	while($arElement = $rsElements->Fetch())
	{
        ...
        global $arRegion;
        $arResult["ELEMENTS"][$arElement["ID"]]["AMOUNT"] = 0;
        $productStoreInfo = CCatalogStoreProduct::GetList(array(), array("STORE_ID" => $arRegion["LIST_STORES"], "PRODUCT_ID" => $arElement["ID"]), false, false, array());
        while ($productInfo = $productStoreInfo -> Fetch()) {
            $arResult["ELEMENTS"][$arElement["ID"]]["AMOUNT"] += $productInfo["AMOUNT"];
        }
	}
