...
<br><div>Чтобы стать участником программы лояльности, заполните анкету и сохраните данные.<br>

Мы создадим электронную бонусную карту, которая будет привязана к номеру вашего мобильного телефона.</div>
<form name="iblock_add" class="iblock_add" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data">
    <?=bitrix_sessid_post()?>
    <div class="formTitle">
        <h2>Анкета участника бонусной программы</h2>
    </div>
    ...
    <table class="data-table" style="width: 90%">
        ...
        <tbody>
            <?foreach ($arResult["PROPERTY_LIST"] as $propertyID):?>
                <tr>
                    <td>
                        <div class="fieldName"><?if (intval($propertyID) > 0):?><?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"]?><?else:?><?=!empty($arParams["CUSTOM_TITLE_".$propertyID]) ? $arParams["CUSTOM_TITLE_".$propertyID] : GetMessage("IBLOCK_FIELD_".$propertyID)?><?endif?></div>
                        <?
                        ...
                        switch ($INPUT_TYPE):
                            case "USER_TYPE":
                                for ($i = 0; $i<$inputNum; $i++)
                                {
                                    ...
                                    if ($arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"] == "BIRTHDAY") {
                                        $blockClass = "birthday";
                                    }?>
                                    <div class="form-control bg">
                                    <div class="wrap_md <?if ($blockClass){ echo $blockClass;}?>">
                                        <div class="iblock label_iblock">
                                    <?
                                ...
                                ?></div>
                                <div class="iblock text_block"></div><br /><?
                                }       
                            break;
                            case "TAGS":
                        ...
                        <textarea cols="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]?>" rows="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"]?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]"><?=$value?></textarea>
                            ...
                            case "N":
                                for ($i = 0; $i<$inputNum; $i++)
                                {
                                    ...
                                    if ($propertyID == "NAME") {
                                        $value = $arResult["NAME_DEFAULT_VALUE"];
                                    }
                                ?>
                                <div class="form-control bg">
                                    <div class="wrap_md">
                                        <div class="iblock label_iblock">
                                            <?/*if ($arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"] == "PHONE_NUMBER") {
                                                $value = $arResult["CURRENT_PHONE_NUMBER"];    
                                            }*/
                                            if (!empty($arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"])) {
                                                $textFieldClass = $arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"];    
                                            } else {
                                                $textFieldClass = $propertyID;
                                            }
                                            ?>
                                            <?if ($arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"] == "PHONE_NUMBER") {?>
                                                <input type="text" name="PROPERTY_PHONE_NUMBER" class="<?= $textFieldClass ?>" size="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]; ?>" value="<?=$value?>" <?if ($disabled) {?>disabled<?}?>/><br />
                                                <br>
                                                <button name="changePhoneNumber" class="btn btn-default short changePhoneNumberButton">Сменить номер телефона</button>                                
                                            <?} else {?>
                                                <input type="text" name="PROPERTY[<?=$propertyID?>][<?=$i?>]" class="<?= $textFieldClass ?>" size="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]; ?>" value="<?=$value?>" <?if ($disabled) {?>disabled<?}?>/><br />    
                                            <?}?>
                                        </div>
                                        <div class="iblock text_block">
                                            <?if (!empty($arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"])) {
                                                echo GetMessage("REGISTER_FIELD_TEXT_".$arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"]);?>
                                            <?} else {
                                                echo GetMessage("REGISTER_FIELD_TEXT_".$propertyID);    
                                            }?>
                                        </div><?
        ...
        <tfoot>
            <tr>
                <td colspan="2">
                    <br>
                    <input type="submit" name="iblock_submit" value="<?=GetMessage("IBLOCK_FORM_SUBMIT")?>" class="btn btn-default short"/>
    ...
    <div class="form">
        ...
        <form action="" method="POST" enctype="multipart/form-data" novalidate="novalidate">
            ...
            <div class="form_body">
                ...
                <input type="text" class="phoneNumber" name="phoneNumber" autofocus>
                <div class="existedPhoneNumber" style="display: none;">Пользователь с данным номером телефона уже существует</div>
                <br><br>
